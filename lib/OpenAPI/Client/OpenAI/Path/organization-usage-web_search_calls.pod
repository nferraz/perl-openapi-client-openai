...
This document describes the API endpoint at C</organization/usage/web_search_calls>.

=head1 PATHS


=head2 C<GET /organization/usage/web_search_calls>
...
C<usage-web-search-calls>

    $client->usage-web-search-calls( ... );
...
=item * C<context_levels> (in query) (Optional) - Return only web search usage for these context levels.

Type: C<array>



=item * C<group_by> (in query) (Optional) - Group the usage data by the specified fields. Support fields include `project_id`, `user_id`, `api_key_id`, `model`, `context_level` or any combination of them.
