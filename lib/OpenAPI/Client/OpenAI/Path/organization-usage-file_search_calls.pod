...
This document describes the API endpoint at C</organization/usage/file_search_calls>.

=head1 PATHS


=head2 C<GET /organization/usage/file_search_calls>
...
C<usage-file-search-calls>

    $client->usage-file-search-calls( ... );
...
=item * C<vector_store_ids> (in query) (Optional) - Return only usage for these vector stores.

Type: C<array>



=item * C<group_by> (in query) (Optional) - Group the usage data by the specified fields. Support fields include `project_id`, `user_id`, `api_key_id`, `vector_store_id` or any combination of them.
